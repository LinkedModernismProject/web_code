...
d3sparql.tree = function(json, config) {
  ...
  for (var i = 0; i < data.length; i++) {
    parent = data[i].name;
    for (var j = 0; j < data[i].children.length; j++) {
      //To determine if there has already been a pred with the same name and then concatenate a space to cope with the way tree implements variable names and adds a space to act as another variable for that tree path
      if(pair.has(data[i].children[j].name)) {//data[i].children[j].name)) {
        ...
        child = data[i].children[j].name;  //keep the new child
      }
      for (var k = 0; k < data[i].children[j].children.length; k++) {
        ...
        if(i==0 && !pair.has(parent)) {  //If this is the first pass, set root var as the root of tree
          ...
          //Just continue rest of for loop
        ...
        if (parent != child) {
          if (pair.has(parent)) {
            ...
            if(children.indexOf(child) == -1) { //if(child!=children) //To stop duplicates of predicates
              ...
              size.set(child, 5); //Doesn't reach this if statement below, so set here
        ...
        //To set the child as the parent of toddler Pred->Objs
        if (child != toddler) {
          if (pair.has(child)) {
            ...
            if(children.indexOf(toddler) == -1) {  //!(toddler in children)) { //To stop duplicates of objects
              ...
              size.set(toddler, 5); //Doesn't reach this if statement below, so set here
              if (data[i][opts.value]) {
                size.set(child, 5);
              }
...
d3sparql.forcegraph = function(json, config) {
  ...
  .linkDistance(300)//opts.distance)
  .gravity(0.05)
