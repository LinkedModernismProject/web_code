...
for line in f:
	if(re.match('^<http://modernism.uvic.ca/metadata#(.*)>', line) is not None):	#re.search('^\s$', line) only for grabbing separator lines
		...
		if mObj:
			...
			modline = mObj.group(1) + mObj.group(2) + ':' + mObj.group(3) + ' "' + m5_objVal + '" ' + mObj.group(6) + '\n'
			fo.write(modline)
		elif mObjStr:
			m4_objVal = mObjStr.group(4).replace('_', ' ')
			modlineStr = mObjStr.group(1) + mObjStr.group(2) + ':' + mObjStr.group(3) + m4_objVal + mObjStr.group(5) + '\n'
			fo.write(modlineStr)
